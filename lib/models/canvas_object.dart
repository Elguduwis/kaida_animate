...
class CanvasObject {
  ...
  CanvasObject({
    String? id, required this.type, required this.data, this.pathData,
    this.x = 50.0, this.y = 50.0, this.width = 200.0, this.height = 100.0,
    this.delay = 0.0, this.duration = 2.0, this.pause = 0.0,
    this.startTime = Duration.zero, this.color = Colors.black, this.fontSize = 40.0,
  }) : id = id ?? const Uuid().v4();

  Map<String, dynamic> toJson() => {
    'id': id, 'type': type.toString(), 'data': data,
    'x': x, 'y': y, 'width': width, 'height': height,
    'delay': delay, 'duration': duration, 'pause': pause,
    'color': color.value, 'fontSize': fontSize,
  };

  factory CanvasObject.fromJson(Map<String, dynamic> json) {
    return CanvasObject(
      id: json['id'],
      type: ObjectType.values.firstWhere((e) => e.toString() == json['type']),
      data: json['data'],
...
class Scene {
  ...
  Scene({String? id, List<CanvasObject>? objects, this.backgroundColor = Colors.white})
      : id = id ?? const Uuid().v4(), objects = objects ?? [];

  Map<String, dynamic> toJson() => {
    'id': id, 'backgroundColor': backgroundColor.value,
