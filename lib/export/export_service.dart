...
class ExportService {
  Future<String?> exportToMp4({
    ...
    try {
        ...
        for (var obj in activeScene.objects) {
          ...
          if (sceneLocalTime >= objStart) {
            ...
            if (obj.type == ObjectType.text) {
              // Benime Style Text Render Export (ClipRect wipe)
              canvas.clipRect(Rect.fromLTWH(0, 0, obj.width * progress, obj.height));
              final textPainter = TextPainter(
                text: TextSpan(text: obj.data, style: TextStyle(color: obj.color, fontSize: obj.fontSize, fontWeight: FontWeight.bold)),
              ...
              for (ui.PathMetric metric in obj.pathData!.computeMetrics()) {
                partialPath.addPath(metric.extractPath(0.0, metric.length * progress), Offset.zero);
              }
      ...
      // PUBLIC FOLDER EXPORT LOGIC
      // Android 11+ allows direct writes to the public Movies directory without extra permissions.
